...
describe('batch-6 #36: hypothesis auto-promote on experiment_plan pairing', () => {
  it('moves an untested hypothesis to testing on a hypothesis_requires_experiment_plan edge', async () => {
    const store = await makeStore()
    const ctx = makeCtx(store)
    ...
    //: hypothesis folded onto VALIDATION — a fresh hypothesis starts
    // untested; pairing it with a plan should move it into testing.
    expect(store.getNode(hyp.node.id)?.status).toBe('untested')
    createEdge(
      { source_id: hyp.node.id, target_id: plan.node.id, type: 'hypothesis_requires_experiment_plan' },
      ctx,
    )
    expect(store.getNode(hyp.node.id)?.status).toBe('testing')
  })
})
