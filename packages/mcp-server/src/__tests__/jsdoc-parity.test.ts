 ...
 * the floor enforced here is the minimum two tags every public handler MUST
 * carry so generated TOOLS.md never regresses.
 *
 ...
 * ToolHandler` line, and asserts both tags appear inside it.
 */
 ...
 * with its preceding JSDoc block. Skips type-only exports and helpers.
 */
function extractExports(filePath: string): Export[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Walk back to find the JSDoc block. JSDoc closes on a line ending in `*/`.
    let j = i - 1
...
describe('JSDoc parity floor on tool handlers', () => {
