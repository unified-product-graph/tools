...
describe('JiraAdapter — issue_type → entity type mapping', () => {
  it('story maps to story_statement with confidence high', async () => {
  ...
  it('"User Story" also maps to story_statement', async () => {
...
describe('JiraAdapter — hierarchy edge emission', () => {
  it('epic_specified_by_story_statement emitted when story has epic parent', async () => {
    const items: SourceItem[] = [
  ...
  it('task_implements_story_statement emitted when sub-task has story parent', async () => {
    const items: SourceItem[] = [
      makeIssue('s1', 'User can log in', 'Story'),
    ...
    const result = await adapter.convert(items)
    assertAllEdgesCatalogued(result.edges, 'task_implements_story_statement')
...
describe('JiraAdapter — full fixture', () => {
  it('all emitted edges are in the UPG catalogue (full delivery tree)', async () => {
    ...
    // epic_specified_by_story_statement (e1→s1)
    // feature_area_contains_feature (c1→s1)
    // release_contains_feature (v1→s1)
    // task_implements_story_statement (s1→t1)
