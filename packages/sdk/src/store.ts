...
export interface MergeResult {
  ...
  /**
   * Shared nodes/edges whose disk-side field changes the three-way merge adopted
   * (0.17.8). Distinct from `nodesFromDisk`/`edgesFromDisk`, which count entities
   * ADDED by the other session. Optional for back-compat with older readers.
   */
  nodesChangedFromDisk?: number
  edgesChangedFromDisk?: number
  /**
   * Genuine conflicts. `nodeId` holds the entity id for either kind (kept for
   * back-compat); `entity` disambiguates node vs edge. Two `kind`s (0.17.8):
   *   - `'field'` (default) — the SAME `field` (a top-level field or
   *     `properties.<key>`) on a shared entity changed to different values by
   *     both sessions; `ours`/`theirs` carry those values.
   *   - `'existence'` — one session DELETED the entity while the other MODIFIED
   *     it (either direction). `field` is `'existence'`; `ours`/`theirs` are the
   *     markers `'(modified)'` / `'(deleted)'` naming what each side did.
   */
  conflicts: Array<{
    nodeId: string
    entity?: 'node' | 'edge'
    kind?: 'field' | 'existence'
    field: string
    ours: unknown
    theirs: unknown
  }>
  /**
   *: the disk copy could not be reconciled at all (unparseable JSON, or
   * a STRUCTURAL/envelope validation failure). Before 0.25.2 both of those
   * returned `merged: true, conflicts: []` — "our version wins" — and
   * `saveLocked()` then serialized the whole in-memory doc over a file it had
   * never read. That is the wholesale-clobber path. The merge now FAILS CLOSED:
   * an unmergeable disk copy is a CONFLICT, never an overwrite.
   */
  unmergeable?: boolean
  /** Reason string for `unmergeable`, surfaced in the CONFLICT message. */
  unmergeableReason?: string
  /**
   * Every node/edge id observed on disk during this merge, plus the disk-side
   * deletions the merge explicitly accepted. `saveLocked()` uses these for the
   * containment invariant: no id that exists on disk may vanish from the doc we
   * are about to write unless its deletion was accepted.
   */
  diskNodeIds?: Set<string>
  diskEdgeIds?: Set<string>
  /** Disk-side edge endpoints, so a legitimate endpoint-cascade can be recognised. */
  diskEdgeEndpoints?: Map<string, { source: string; target: string }>
  acceptedNodeDeletions?: Set<string>
  acceptedEdgeDeletions?: Set<string>
}

// ── Field-level three-way merge helpers (0.17.8) ─────────────────────────────

interface FieldMergeResult {
  /** ours + adopted disk-side one-sided changes (only meaningful when no conflicts). */
  merged: Record<string, unknown>
  /** fields/keys the two sessions changed differently. */
  conflicts: Array<{ field: string; ours: unknown; theirs: unknown }>
  /** true when at least one disk-side change was adopted (replacement needed). */
...
/**
 * Deterministic serialization for deep equality: object keys sorted (so a
 * property object built in a different key order compares equal), arrays kept in
 * order (array order is semantically meaningful, e.g. `aliases[]`). `undefined`
 * (an absent field) maps to a sentinel that no real value can produce.
 */
...
/** Order-insensitive (object-key) deep equality; absent/undefined compare equal. */
...
/**
 * Field-level three-way merge for one shared entity (node or edge). Every
 * top-level field is reconciled whole; `properties` is reconciled at the KEY
 * level so two sessions touching DIFFERENT property keys never conflict; `id` is
 * immutable and never reconciled. Rule per field/key `f` with base/ours/disk:
 *   - `ours == disk`              → agree, keep (whether or not either changed)
 *   - `ours == base, disk ≠ base` → other session changed it → take disk
 *   - `disk == base, ours ≠ base` → we changed it → keep ours
 *   - both ≠ base and ours ≠ disk → genuine CONFLICT (surface for ANY field)
 * A field whose disk value is absent (deleted by the other session) while we held
 * the baseline is dropped from the merged entity.
 */
function mergeEntityFields(
  ...
  let changedFromDisk = false
  const merged: Record<string, unknown> = { ...ours }

  const topKeys = new Set<string>([...Object.keys(base), ...Object.keys(ours), ...Object.keys(disk)])
  topKeys.delete('id')
  topKeys.delete('properties')

  for (const f of topKeys) {
    const b = base[f]
    const o = ours[f]
    const d = disk[f]
    if (stableEqual(o, d)) continue
    if (stableEqual(o, b)) {
      // other session changed it; we did not → take disk
      if (d === undefined) delete merged[f]
      else merged[f] = d
      changedFromDisk = true
    } else if (stableEqual(d, b)) {
      // we changed it; other session did not → keep ours (already in merged)
    } else {
      conflicts.push({ field: f, ours: o, theirs: d })
    }
  }

  // `properties`: reconcile at the property-KEY level.
  const baseP = asRecord(base.properties)
  const ourP = asRecord(ours.properties)
  const diskP = asRecord(disk.properties)
  if (baseP || ourP || diskP) {
    const bp = baseP ?? {}
    const op = ourP ?? {}
    const dp = diskP ?? {}
    const mergedP: Record<string, unknown> = { ...op }
    const propKeys = new Set<string>([...Object.keys(bp), ...Object.keys(op), ...Object.keys(dp)])
    for (const k of propKeys) {
      ...
      if (stableEqual(o, d)) continue
      if (stableEqual(o, b)) {
        if (d === undefined) delete mergedP[k]
        else mergedP[k] = d
        changedFromDisk = true
      } else if (stableEqual(d, b)) {
        // keep ours
      } else {
        conflicts.push({ field: `properties.${k}`, ours: o, theirs: d })
      }
    }
    if (Object.keys(mergedP).length > 0) merged.properties = mergedP
...
export class UPGFileStore {
  ...
  // Snapshot of node/edge CONTENT at baseline (0.17.8), used for FIELD-LEVEL
  // three-way merge. Deep-cloned at load and after every save so the merge can
  // tell "we changed field X" (ours ≠ base) from "we hold the baseline X while
  // the other session changed it" (ours == base, disk ≠ base). Without this the
  // merge could only punt to title/status + keep-ours, silently clobbering any
  // other field the other session touched. Cost: one extra in-memory copy of the
  // graph (the doc is already held once) plus an O(n) clone per save. For UPG's
  // target graph sizes (hundreds–low thousands of entities) this is negligible;
  // for very large graphs it is a bounded 2× with an O(n) clone on the write
  // path. If that ever bites, swap the clone for per-field content hashes (same
  // asymptotics, ~half the memory) without changing the merge rule below.
  private baselineNodes = new Map<string, UPGBaseNode>()
  private baselineEdges = new Map<string, UPGEdge>()
  ...
  /**
   * Snapshot current node/edge IDs AND content as the baseline for the
   * field-level three-way merge (0.17.8). The content clones must be deep so a
   * later in-place mutation of a live node/edge (e.g. `updateNode`, which mutates
   * the shared object) does not also mutate the baseline — otherwise the merge
   * could never detect a local change.
   */
  private snapshotBaseline(): void {
    ...
    this.baselineNodes = new Map(this.doc.nodes.map((n) => [n.id, structuredClone(n)]))
    this.baselineEdges = new Map(this.doc.edges.map((e) => [e.id, structuredClone(e)]))
  }
  ...
  private async loadInternal(filePath: string, opts: { watch: boolean }): Promise<void> {
    ...
    // ── I-1: symmetric refusal. A portfolio document is NEVER held by
    // UPGFileStore. ─────────────────────────────────────────────────────────
    // The converse of `UPGPortfolioStore.loadOrInit()`'s guard (which throws on
    // `type !== 'portfolio'`). Until this landed, this side failed only
    // INCIDENTALLY — `validateUPGDocument` returned "product is required", the
    // same dead-end schema error the 0.25.0 directed error was written to paper
    // over downstream in the MCP layer. Incidental refusal is not a mechanism:
    // it is contingent on a validator rule that a future permissive-load change
    // could relax, and it names the wrong problem.
    //
    // Why refusing matters more than it looks. A UPGFileStore that ACCEPTED a
    // portfolio document would not merely race the portfolio writer — on save it
    // would serialize the document through the SINGLE-PRODUCT serializer,
    // emitting `product` + `nodes` + `edges` and dropping `organization`,
    // `product_areas`, `portfolios`, `products`, `registry` and `cross_edges` on
    // the floor. `$upg.integrity` is recomputed by `serializeCanonical` on every
    // write, so the hash would be re-stamped over the wreckage and the resulting
    // file would present as perfectly intact. The hash does not catch this class
    // of writer; it certifies it. Hence a type check at the load boundary, which
    // is the only place the two document kinds are still distinguishable.
    //
    // `normalizeDocument` classifies a portfolio via `type === 'portfolio'`,
    // `$upg.kind === 'portfolio'`, or the presence of `cross_edges`, and always
    // stamps `type: 'portfolio'` on its output — so this single check covers all
    // three detection routes and stays in step with the normaliser by
    // construction.
    if ((parsed as { type?: string }).type === 'portfolio') {
      throw new Error(
        `Refusing to load a PORTFOLIO document as a product: ${this.filePath}\n` +
          `This file is a portfolio index (organization / product_areas / portfolios / ` +
          `products / registry / cross_edges), not a single-product graph. It must be ` +
          `held by UPGPortfolioStore, which owns its compare-and-swap write path.\n` +
          `Loading it here would serialize it through the single-product serializer on ` +
          `the next save, silently discarding every portfolio-only section.\n` +
          `Use \`new UPGPortfolioStore().loadOrInit(path)\` instead.`,
      )
    }
  ...
  private async saveLocked(): Promise<void> {
    ...
    try {
      ...
      //: ONLY a genuinely absent file is "safe to write". The previous
      // bare `catch { diskRaw = '' }` swallowed EACCES/EBUSY/EIO too, and an
      // unreadable file then took the same branch as a deleted one: the freshness
      // check was skipped entirely and the in-memory doc was written wholesale
      // over a file whose contents we had just failed to read. A read we cannot
      // complete is not evidence of freshness — fail closed.
      const code = (err as NodeJS.ErrnoException).code
      if (code !== 'ENOENT') {
        throw new Error(
          `CONFLICT: could not read ${this.filePath} to verify it is unchanged before saving ` +
          `(${code ?? 'read error'}: ${(err as Error).message}).\n` +
          `Refusing to write — an unverifiable file must never be overwritten wholesale.\n` +
          `Resolve the read error, then reload_product({ discard_local: true }) and redo the change.`,
        )
      }
      diskRaw = ''
    }
    ...
    if (diskHash && diskHash !== this.baselineFileHash) {
      ...
      if (this.lastMergeResult.unmergeable) {
        //: the disk copy exists and differs from our baseline but cannot
        // be reconciled. Writing here would discard whatever is in it.
        throw new Error(
          `CONFLICT: The .upg file was modified by another session and cannot be merged — ` +
          `${this.lastMergeResult.unmergeableReason}.\n` +
          `Refusing to write: saving would overwrite the on-disk file wholesale with this ` +
          `session's in-memory state and destroy the other session's work.\n` +
          `Repair the file on disk, then reload_product({ discard_local: true }) and redo the change.`,
        )
      }
      ...
      if (this.lastMergeResult.conflicts.length > 0) {
        ...
        const conflictDesc = this.lastMergeResult.conflicts
          .map((c) => {
            const label = c.entity === 'edge' ? 'Edge' : 'Node'
            if (c.kind === 'existence') {
              return `  ${label} ${c.nodeId}: deleted by one session, modified by the other (ours: ${c.ours}, theirs: ${c.theirs})`
            }
            return `  ${label} ${c.nodeId}: field "${c.field}": ours: ${JSON.stringify(c.ours)}, theirs: ${JSON.stringify(c.theirs)}`
          })
          .join('\n')
        throw new Error(
      ...
      // ── Layer 3: containment invariant ──────────────────────────
      // Belt-and-braces over the merge. Whatever the merge decided, the bytes we
      // are about to write MUST still contain every entity that is on disk right
      // now, minus only the deletions the merge explicitly accepted. A merge
      // defect, a future refactor, or an unforeseen shape can no longer express
      // itself as silent data loss: it becomes a loud CONFLICT with the file on
      // disk untouched. This is the invariant that would have caught the
      // 2026-08-05 incident regardless of which code path caused it.
      this.assertNoDiskEntitiesDropped(this.lastMergeResult)
    }
  ...
  /**
   *: the containment invariant enforced on every product save that
   * followed a merge. Throws `CONFLICT` — leaving the on-disk file byte-identical
   * — if the document we are about to serialize would drop any node or edge that
   * currently exists on disk WITHOUT this session ever having seen it.
   *
   * A disk entity may legitimately be absent from the bytes we write in exactly
   * three cases:
   *   1. it is in our BASELINE — we loaded it, then deliberately deleted it, and
   *      the merge already vetted that deletion against the other session;
   *   2. the merge explicitly accepted the other session's deletion of it;
   *   3. (edges) one of its endpoints is a node legitimately disappearing, so the
   *      edge cascades out with it.
   *
   * Anything else is an entity the other session created that we never observed.
   * Dropping it is pure data loss — the 2026-08-05 shape exactly — and aborts the
   * write regardless of which code path produced it.
   */
  private assertNoDiskEntitiesDropped(result: MergeResult | null): void {
    if (!result?.diskNodeIds || !result.diskEdgeIds) return
    const acceptedNodes = result.acceptedNodeDeletions ?? new Set<string>()
    const acceptedEdges = result.acceptedEdgeDeletions ?? new Set<string>()

    const ourNodeIds = new Set(this.doc.nodes.map((n) => n.id))
    const goneNodeIds = new Set<string>()
    const droppedNodes: string[] = []
    for (const id of result.diskNodeIds) {
      if (ourNodeIds.has(id)) continue
      goneNodeIds.add(id)
      if (this.baselineNodeIds.has(id) || acceptedNodes.has(id)) continue
      droppedNodes.push(id)
    }

    const ourEdgeIds = new Set(this.doc.edges.map((e) => e.id))
    const droppedEdges: string[] = []
    for (const [id, endpoints] of result.diskEdgeEndpoints ?? new Map()) {
      if (ourEdgeIds.has(id)) continue
      if (this.baselineEdgeIds.has(id) || acceptedEdges.has(id)) continue
      // Cascade: an edge whose endpoint is legitimately gone goes with it.
      if (goneNodeIds.has(endpoints.source) || goneNodeIds.has(endpoints.target)) continue
      droppedEdges.push(id)
    }

    if (droppedNodes.length === 0 && droppedEdges.length === 0) return

    const sample = (ids: string[]) =>
      ids.slice(0, 10).join(', ') + (ids.length > 10 ? `, …(+${ids.length - 10} more)` : '')
    const parts: string[] = []
    if (droppedNodes.length > 0) parts.push(`  ${droppedNodes.length} node(s): ${sample(droppedNodes)}`)
    if (droppedEdges.length > 0) parts.push(`  ${droppedEdges.length} edge(s): ${sample(droppedEdges)}`)

    throw new Error(
      `CONFLICT: refusing to save — the write would remove entities that exist on disk ` +
      `but were never deleted by this session:\n${parts.join('\n')}\n\n` +
      `This is the load-before-save containment invariant. The file on disk is ` +
      `unchanged. Run reload_product({ discard_local: true }) to adopt the current file, ` +
      `then redo this change.`,
    )
  }
  ...
  //
  // Three states:
  //   baseline = what was on disk when we loaded (or last saved), by ID *and*
  //              CONTENT (0.17.8: `baselineNodes`/`baselineEdges`)
  //   disk     = what's on disk now (another session wrote this)
  //   ours     = our in-memory state
  //
  // Membership strategy (by ID vs baseline):
  //   - in disk, not in baseline → added by other session → adopt
  //   - in ours, not in baseline → added by this session → keep
  //   - in baseline, not in disk → deleted by other session → accept deletion
  //   - in baseline, not in ours → deleted by this session → deletion stands
  //   - in all three (shared)    → FIELD-LEVEL three-way merge (mergeEntityFields):
  //       adopt the other session's one-sided field changes, keep ours where only
  //       we changed, and surface a CONFLICT only where BOTH changed the SAME
  //       field to different values. Before 0.17.8 this branch compared only
  //       title/status, silently clobbering every other field the other session
  //       touched (and silently one-winning a genuine non-title/status conflict).
  //
  // The plan is computed WITHOUT mutating `this.doc`; it is applied only when the
  // merge is conflict-free, so a thrown CONFLICT leaves our in-memory state clean
  // for `reload_product` recovery.
  private async mergeWithDisk(diskRaw: string): Promise<MergeResult> {
    const unmergeable = (reason: string): MergeResult => ({
      merged: false,
      nodesAdded: 0,
      edgesAdded: 0,
      nodesFromDisk: 0,
      ...
      conflicts: [],
      unmergeable: true,
      unmergeableReason: reason,
    })

    let diskDoc: UPGDocument
    try {
      const parsed = normalizeDocument(JSON.parse(diskRaw))
      //: classify exactly as `loadInternal()` and `refreshFromDisk()` do.
      // The load boundary is deliberately PERMISSIVE about content-validity (a
      // stale enum on one node still loads); using the strict `.valid` here made
      // the merge boundary strict AND fail-OPEN, so a single content-validity
      // error on a freshly written disk copy — routine when two servers run
      // different spec versions against the same file — silently downgraded to
      // "our version wins" and the stale doc was written over it. Content drift
      // must MERGE; only a structural/envelope failure is unmergeable, and that
      // now blocks the write instead of licensing it.
      const validation = validateUPGDocument(parsed)
      if (!validation.valid) {
        const { structural } = this.classifyValidationErrors(validation.errors)
        if (structural.length > 0) {
          return unmergeable(
            `the on-disk .upg is not a well-formed UPG document ` +
            `(${structural.length} structural ${structural.length === 1 ? 'error' : 'errors'}: ` +
            `${structural.map((e) => `${e.path}: ${e.message}`).join('; ')})`,
          )
        }
      }
      ...
      return unmergeable(
        `the on-disk .upg could not be parsed (${(err as Error).message})`,
      )
    }
    ...
    const nodesToAdd: UPGBaseNode[] = []
    const edgesToAdd: UPGEdge[] = []
    const nodeReplacements = new Map<string, UPGBaseNode>()
    const edgeReplacements = new Map<string, UPGEdge>()
    const nodeIdsToDelete = new Set<string>()
    const edgeIdsToDelete = new Set<string>()
    let nodesFromDisk = 0
    let edgesFromDisk = 0
    let nodesChangedFromDisk = 0
    let edgesChangedFromDisk = 0

    // Nodes: adds by the other session + field-level merge of shared nodes.
    for (const [id, diskNode] of diskNodeMap) {
      if (!this.baselineNodeIds.has(id)) {
        if (!ourNodeMap.has(id)) {
          nodesToAdd.push(diskNode)
          nodesFromDisk++
        }
      } else if (ourNodeMap.has(id)) {
        const ourNode = ourNodeMap.get(id)!
        const baseNode = this.baselineNodes.get(id) ?? ourNode
        const res = mergeEntityFields(
          baseNode as unknown as Record<string, unknown>,
          ourNode as unknown as Record<string, unknown>,
          diskNode as unknown as Record<string, unknown>,
        )
        for (const fc of res.conflicts) {
          conflicts.push({ nodeId: id, entity: 'node', field: fc.field, ours: fc.ours, theirs: fc.theirs })
        }
        if (res.conflicts.length === 0 && res.changedFromDisk) {
          nodeReplacements.set(id, res.merged as unknown as UPGBaseNode)
          nodesChangedFromDisk++
        }
      } else {
        // In baseline & disk but NOT in ours → WE deleted it (9b). If the other
        // session MODIFIED it (disk ≠ baseline), our deletion would silently drop
        // their committed edit → CONFLICT. A clean delete (disk == baseline, they
        // did not touch it) stands silently.
        const baseNode = this.baselineNodes.get(id)
        if (baseNode !== undefined && !stableEqual(diskNode, baseNode)) {
          conflicts.push({ nodeId: id, entity: 'node', kind: 'existence', field: 'existence', ours: '(deleted)', theirs: '(modified)' })
        }
      }
    }

    // Edges: adds by the other session + field-level merge of shared edges.
      ...
      if (!this.baselineEdgeIds.has(id)) {
        if (!ourEdgeMap.has(id)) {
          ...
          if (sourceExists && targetExists) {
            edgesToAdd.push(diskEdge)
            edgesFromDisk++
          }
        }
      } else if (ourEdgeMap.has(id)) {
        const ourEdge = ourEdgeMap.get(id)!
        const baseEdge = this.baselineEdges.get(id) ?? ourEdge
        const res = mergeEntityFields(
          baseEdge as unknown as Record<string, unknown>,
          ourEdge as unknown as Record<string, unknown>,
          diskEdge as unknown as Record<string, unknown>,
        )
        for (const fc of res.conflicts) {
          conflicts.push({ nodeId: id, entity: 'edge', field: fc.field, ours: fc.ours, theirs: fc.theirs })
        }
        if (res.conflicts.length === 0 && res.changedFromDisk) {
          edgeReplacements.set(id, res.merged as unknown as UPGEdge)
          edgesChangedFromDisk++
        }
      } else {
        // In baseline & disk but NOT in ours → WE deleted the edge (9b). Same
        // rule as nodes: a modify by the other session (disk ≠ baseline) makes
        // our deletion a CONFLICT; a clean delete stands silently.
        const baseEdge = this.baselineEdges.get(id)
        if (baseEdge !== undefined && !stableEqual(diskEdge, baseEdge)) {
          conflicts.push({ nodeId: id, entity: 'edge', kind: 'existence', field: 'existence', ours: '(deleted)', theirs: '(modified)' })
        }
      }
    }

    // Node deletions by the other session (baseline ∩ ours, not on disk) — 9a.
    // If WE modified the node (content ≠ baseline) while the other session
    // deleted it, that is a delete-vs-modify CONFLICT (0.17.8: was silently
    // keep-ours, resurrecting the node). A clean delete (we did not touch it) is
    // accepted silently. Content-compare against the baseline is used rather than
    // the changeLog so a no-op update (edited then reverted) does not block a
    // clean deletion.
    for (const id of this.baselineNodeIds) {
      if (!diskNodeMap.has(id) && ourNodeMap.has(id)) {
        const ourNode = ourNodeMap.get(id)!
        const baseNode = this.baselineNodes.get(id)
        const weModified = baseNode !== undefined && !stableEqual(ourNode, baseNode)
        if (weModified) {
          conflicts.push({ nodeId: id, entity: 'node', kind: 'existence', field: 'existence', ours: '(modified)', theirs: '(deleted)' })
        } else {
    ...
    // Edge deletions by the other session (baseline ∩ ours, not on disk) — 9a.
    // A re-parent is delete-old-edge + add-new-edge. If WE also modified the
    // deleted edge, that is a delete-vs-modify CONFLICT (0.17.8: this closes the
    // "modify + re-parent → silent double-parent" path — the conflict aborts the
    // whole apply, so the added new edge is not adopted alongside our kept copy).
    // A clean delete (we did not touch it) is accepted silently.
    for (const id of this.baselineEdgeIds) {
      if (!diskEdgeMap.has(id) && ourEdgeMap.has(id)) {
        const ourEdge = ourEdgeMap.get(id)!
        const baseEdge = this.baselineEdges.get(id)
        const weModified = baseEdge !== undefined && !stableEqual(ourEdge, baseEdge)
        if (weModified) {
          conflicts.push({ nodeId: id, entity: 'edge', kind: 'existence', field: 'existence', ours: '(modified)', theirs: '(deleted)' })
        } else {
    ...
    // ── Apply the plan — only when conflict-free ──────────────────────────────
    if (conflicts.length === 0) {
      if (nodesToAdd.length > 0) this.doc.nodes.push(...nodesToAdd)
      if (edgesToAdd.length > 0) this.doc.edges.push(...edgesToAdd)
      if (nodeReplacements.size > 0) {
        this.doc.nodes = this.doc.nodes.map((n) => nodeReplacements.get(n.id) ?? n)
      }
      if (edgeReplacements.size > 0) {
        this.doc.edges = this.doc.edges.map((e) => edgeReplacements.get(e.id) ?? e)
      }
      if (nodeIdsToDelete.size > 0) {
        this.doc.nodes = this.doc.nodes.filter((n) => !nodeIdsToDelete.has(n.id))
        // Also remove edges connected to a deleted node.
        this.doc.edges = this.doc.edges.filter(
          (e) => !nodeIdsToDelete.has(e.source) && !nodeIdsToDelete.has(e.target),
        )
      }
      if (edgeIdsToDelete.size > 0) {
        this.doc.edges = this.doc.edges.filter((e) => !edgeIdsToDelete.has(e.id))
      }
    }
    ...
    return {
      ...
      edgesChangedFromDisk,
      conflicts,
      diskNodeIds: new Set(diskNodeMap.keys()),
      diskEdgeIds: new Set(diskEdgeMap.keys()),
      diskEdgeEndpoints: new Map(
        [...diskEdgeMap].map(([id, e]) => [id, { source: e.source, target: e.target }]),
      ),
      acceptedNodeDeletions: nodeIdsToDelete,
      acceptedEdgeDeletions: edgeIdsToDelete,
    }
  }
  ...
  private scheduleSave(): void {
    ...
    //: a refused save is NOT transient — it means the file changed under
    // us and we declined to clobber it. Silence there would look like a
    // successful write, so surface it on stderr; the in-memory state is retained
    // and `reload_product({ discard_local: true })` is the way out.
    this.saveTimer = setTimeout(
      () => void this.save().catch((err: Error) => this.reportDeferredSaveFailure(err)),
      300,
    )
  }

  /** Surface a background (debounced) save failure without crashing the process. */
  private reportDeferredSaveFailure(err: Error): void {
    if (!/^CONFLICT/.test(err.message)) return
    process.stderr.write(
      `[upg-save] A background save of ${this.filePath} was REFUSED to protect the file:\n` +
      `${err.message}\n` +
      `Unsaved changes are still held in memory and were NOT written.\n`,
    )
  }
  ...
  /**
   * Adopt an external change to the backing file, if there is one.
   *
   * This is the body the chokidar `change` handler runs, exposed so a caller
   * that cannot rely on the watcher can drive the same logic itself. Two real
   * cases need that (both R-1):
   *
   *  - **The ready race.** chokidar's initial scan is async. A write that lands
   *    between `watch()` and `ready` is folded into the scan as the file's
   *    baseline and never surfaces as a `change` event, so the store silently
   *    keeps its pre-write state forever.
   *  - **Duplicated module registries.** A host that instantiates this module
   *    more than once (Next dev/Turbopack across compilation units) can end up
   *    serving reads from a store instance whose watcher belongs to a different
   *    copy of the graph.
   *
   * A caller polling cheap disk metadata (mtime/size) and calling this on
   * change is immune to both. Safe and cheap to call redundantly: a self-write
   * or an unchanged file costs one read + one hash and returns false.
   *
   * @returns true when disk content was actually adopted or merged in.
   */
  async refreshFromDisk(): Promise<boolean> {
    ...
    try {
      ...
      if (this.hashRawContent(raw) === this.baselineFileHash) return false
      // R-1: normalise BEFORE validating, exactly as loadInternal() and
      // mergeWithDisk() do. On-disk canonical form is the `$upg` envelope;
      // validateUPGDocument only understands the lifted flat shape, so
      // validating the raw parse reported missing `$.upg_version` / `$.source`
      // for EVERY modern `.upg` file and this handler returned early — the
      // external-change path was dead for all real graphs, silently.
      const parsed = normalizeDocument(JSON.parse(raw))
      // Permissiveness parity with load(): only a structural/envelope problem
      // means "not a UPG document" and is worth ignoring the change over.
      // Content drift (a stale enum on one node) must not freeze hot-reload —
      // that would re-create the same silent staleness one spec bump later.
      const validation = validateUPGDocument(parsed)
      if (!validation.valid) {
        const { structural } = this.classifyValidationErrors(validation.errors)
      ...
      if (this.dirty) {
        ...
        const reconciled =
          result.nodesFromDisk > 0 ||
          result.edgesFromDisk > 0 ||
          (result.nodesChangedFromDisk ?? 0) > 0 ||
          (result.edgesChangedFromDisk ?? 0) > 0
        if (result.conflicts.length === 0 && reconciled) {
          // Merge succeeded (adds and/or adopted field changes); rebuild indexes,
          // keep our dirty flag so we save the merged state.
          this.rebuildIndexes()
          this.computeHash()
          this.baselineFileHash = this.hashRawContent(raw)
...
export class UPGPortfolioStore {
  ...
  private saveTimer: ReturnType<typeof setTimeout> | null = null
  /**
   *: hash of the exact bytes this store last read from / wrote to disk.
   * `''` means "we have never seen a file at this path" (fresh init). Every write
   * re-reads the file and compares; a mismatch aborts the write.
   */
  private baselineFileHash = ''

  /** Hash raw file bytes for the compare-and-swap check. */
  private hashRawContent(raw: string): string {
    return createHash('sha256').update(raw).digest('hex').slice(0, 32)
  }
  ...
  private scheduleSave(): void {
    ...
    //: as with the product store, a CONFLICT here is a refusal to
    // clobber, not a transient FS blip — never let it pass silently.
    this.saveTimer = setTimeout(
      () =>
        void this.writeToDisk().catch((err: Error) => {
          if (!/^CONFLICT/.test(err.message)) return
          process.stderr.write(
            `[upg-save] A background save of ${this.filePath} was REFUSED to protect the file:\n` +
            `${err.message}\nUnsaved portfolio changes are still held in memory.\n`,
          )
        }),
      300,
    )
  ...
  /**
   *: re-read the portfolio file and reconcile this store with it,
   * discarding unsaved in-memory changes. The in-band recovery from a portfolio
   * CONFLICT — the counterpart of `reload_product` for the product store.
   */
  async reload(): Promise<PortfolioLoadResult> {
    if (!this.filePath) throw new Error('Portfolio document not loaded. Call loadOrInit() first.')
    if (this.saveTimer) {
      clearTimeout(this.saveTimer)
      this.saveTimer = null
    }
    this.dirty = false
    return this.loadOrInit(this.filePath)
  }

  private async writeToDisk(): Promise<void> {
    if (!this.dirty || !this.doc || !this.filePath) return
    // ──: serialize the read-modify-write window ─────────────────────
    // Portfolio writes previously took no lock and made no freshness check: the
    // whole in-memory doc was serialized straight over whatever was on disk. Two
    // servers (or two tool calls) holding the same portfolio therefore lost each
    // other's work silently. Mirror the product store: hold the advisory lock
    // across read-compare-write, keyed on the resolved path.
  ...
  /**
   * The compare-and-swap write. MUST run under the advisory lock held by
   * `writeToDisk()`. Refuses — loudly, with the file untouched — if the bytes on
   * disk are not the bytes this store last read or wrote. There is deliberately
   * NO merge here: a portfolio document is a small wholesale structure, and
   * guessing at a reconciliation is exactly how the in-memory copy wins over
   * fresher work. Fail closed and make the caller reload.
   */
  private async writeLocked(): Promise<void> {
    if (!this.doc || !this.filePath) return
    ...
    try {
      ...
      if (code !== 'ENOENT') {
        throw new Error(
          `CONFLICT: could not read ${this.filePath} to verify it is unchanged before saving ` +
          `(${code ?? 'read error'}: ${(err as Error).message}). Refusing to write.`,
        )
      }
      diskRaw = null
    }

    const diskHash = diskRaw === null ? '' : this.hashRawContent(diskRaw)
    if (diskHash !== this.baselineFileHash) {
      const what =
        diskRaw === null
          ? 'the portfolio file was deleted by another session'
          : this.baselineFileHash === ''
            ? 'another session created the portfolio file after this store initialised it'
            : 'the portfolio file was modified by another session'
      throw new Error(
        `CONFLICT: ${what} (${this.filePath}).\n` +
        `Refusing to write: this store's in-memory portfolio is stale and saving it would ` +
        `overwrite the other session's work wholesale.\n` +
        `Reload the portfolio and redo the change.`,
      )
    }
    ...
    this.dirty = false
    //: adopt the bytes we just wrote as the new compare-and-swap
    // baseline, synchronously after the rename, so our own write is never
    // mistaken for an external one on the next save.
    this.baselineFileHash = this.hashRawContent(output)
  }
  ...
  addCrossEdge(
    ...
    // 3-state cross-product gate (0.18.0), the authoritative write backstop:
    // `curated` and `provisional` (unflagged catalog edge whose ≥1 endpoint is
    // portfolio-shared) persist; `resident` (no shared endpoint) is hard-rejected.
    // The provisional warning is surfaced by the calling tool/CLI, not the store.
    if (crossProductScope(edge.type) === 'resident') {
      throw new Error(
        `Cross-product edge type "${edge.type}" is not authorable across product graphs: ` +
        `neither endpoint type is portfolio-shared (a within-graph/resident relationship).`,
      )
    }
