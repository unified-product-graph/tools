 ...
 *
 * Lives in the shared tooling package so BOTH the local file-backed server and
 * the cloud Postgres-backed server assemble identical trees from the same code.
 * It is pure over a minimal {@link GraphReader}: the local `UPGFileStore`
 * satisfies it structurally; the cloud handler builds an in-memory reader from a
 * one-shot node+edge load of the product.
 *
